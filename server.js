...
function createNewComment(url, request) {
  ...
  if(newComment && newComment.body && database.users[newComment.username] && database.articles[newComment.articleId]) {
  ...
  const user = database.users[comment.username];
  user.commentIds.push(comment.id);
  const article = database.articles[comment.articleId];
  article.commentIds.push(comment.articleId);
...
const response = {};
const upvotedComment = database.comments[id];

if(!database.comments[id] || !database.users[username]) {
  response.status = 400;
}

else {

upvote(upvotedComment, username);
response.status = 200;
...
function downvoteComment(url, request) {
  ...
  const username = request.body && request.body.username;
  const response = {};
  const downvotedComment = database.comments[id];

  if (!database.comments[id] || !database.users[username]) {
    response.status = 400;
  } else {

  downvote(downvotedComment, username);
  response.status = 200;
